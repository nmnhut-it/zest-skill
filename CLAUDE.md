...
|---------|-----------|-------------|
| "setup tools", "install" | `skills/auto-install.md` | Auto-install analysis tools (no Maven needed) |
...
**Semgrep** (works on source code, no compile):
```bash
pip install semgrep
...
**Run without Maven:**
```bash
# Semgrep (recommended - no Java needed)
semgrep --config "p/java" --config "p/owasp-top-ten" --json ./src
...
```
1. Semgrep (pip) → Works everywhere, no Java needed
2. PMD standalone → Needs Java runtime, no compile
3. SpotBugs standalone → Needs compiled .class files
4. Docker images → If nothing else works
5. Grep patterns → Last resort, basic detection
```
...
Pass 1 (Static)  →  Pass 2 (AI Focus)  →  Pass 3 (Deep)
   Semgrep          High-weight           Security
   PMD              uncovered             Performance
   (SpotBugs)       regions               AI-smell
                                          Logging
```

**Minimum required**: Semgrep only (can run full analysis)
